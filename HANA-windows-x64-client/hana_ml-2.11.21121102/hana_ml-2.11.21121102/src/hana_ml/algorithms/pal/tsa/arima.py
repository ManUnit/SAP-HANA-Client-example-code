...
class ARIMA(_ARIMABase):
    ...
    background_size : int, optional
        ...
        If you want to use the ARIMA with explainations, you must set ``background_size`` to be larger than 0 or -1(auto mode)
    ...
    def predict(self,
        ...
        show_explainer : bool, optional
            ...
            Only valide when ``backgroud_size`` is set when initializing an ARIMA instance.
        ...
        if ((self.background_size is None) or (self.background_size == 0)) and (show_explainer is True):
            msg = 'please re-fit the model using parameter "background_size" which need to be non-zero value and then show the explainer in the predict!'
            logger.error(msg)
            raise ValueError(msg)
